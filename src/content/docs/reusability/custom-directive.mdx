...
We have introduced two forms of code reuse in Vue: <span class="custom-link">[components](https://vuejs.org/guide/essentials/component-basics.html)</span> and <span class="custom-link">[composables](/reusability/composable/)</span>. Components are the main building blocks, while composables are focused on reusing stateful logic. Custom directives, on the other hand, are mainly intended for reusing logic that involves low-level DOM access on plain elements.
...
Apart from `el`, you should treat these arguments as read-only and never modify them. If you need to share information across hooks, it is recommended to do so through element's <span class="custom-link">[dataset](https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/dataset)</span>.
...
<Aside type="caution" title="Not recommended">
  Using custom directives on components is not recommended. Unexpected behaviour
  may occur when a component has multiple root nodes.
</Aside>

When used on components, custom directives will always apply to a component's root node, similar to <span class="custom-link">[Fallthrough Attributes](/components/attribute/)</span>.
