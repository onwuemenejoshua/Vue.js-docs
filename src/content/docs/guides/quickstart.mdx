...
You should now have your first Vue project running! Note that the example components in the generated project are written using the [Composition API](/guides/introduction/#composition-api) and `<script setup>`, rather than the [Options API](/guides/introduction/#options-api). Here are some additional tips:
...
Throughout the rest of the documentation, we will be primarily using ES modules syntax. Most modern browsers now support [ES modules](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules) natively, so we can use Vue from a CDN via native ES modules like this:
...
If you skipped the [Introduction](/guides/introduction/), we strongly recommend reading it before moving on to the rest of the documentation
