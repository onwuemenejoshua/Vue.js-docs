...
By leveraging the ability to target a particular prop and event as we learned before with [`v-model` arguments](/components/v-model/#v-model-arguments), we can now create multiple `v-model` bindings on a single component instance.
