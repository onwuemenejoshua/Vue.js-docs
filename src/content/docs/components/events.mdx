...
If you're using an explicit `setup` function instead of `<script setup>`, events should be declared using the emits option, and the [`emit`](https://vuejs.org/api/options-state#emits) function is exposed on the `setup()` context:
...
More details: [Typing Component Emits](https://vuejs.org/guide/typescript/composition-api#typing-component-emits) <sup>TS</sup>
