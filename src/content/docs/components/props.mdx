---
title: Props
description: The page provides a comprehensive guide on how to declare and manage props in Vue.js components.
---
...
Vue components require explicit props declaration so that Vue knows what external props passed to the component should be treated as fallthrough attributes (which will be discussed in <span class="custom-link">[its dedicated section](/components/attribute/)</span>).
...
In non-`<script setup>` components, props are declared using the <span class="custom-link">[props](https://vuejs.org/api/options-state.html#props)</span> option:
...
This not only documents your component, but will also warn other developers using your component in the browser console if they pass the wrong type. We will discuss more details about <span class="custom-link">[prop validation](/components/props/#prop-validation)</span> further down this page.
...
More details: <span class="custom-link">[Typing Component Props](https://vuejs.org/guide/typescript/composition-api.html#typing-component-props)</span> <sup>TS</sup>

## Reactive Props Destructure ​<sup>3.5+</sup>
...
The external function can call the getter (or normalize it with <span class="custom-link">[toValue](https://vuejs.org/api/reactivity-utilities.html#tovalue)</span>) when it needs to track changes of the provided prop, e.g. in a computed or watcher getter.
...
Technically, you can also use camelCase when passing props to a child component (except in <span class="custom-link">[in-DOM templates](https://vuejs.org/guide/essentials/component-basics.html#in-dom-template-parsing-caveats)</span>). However, the convention is using kebab-case in all cases to align with HTML attributes:

```html
<MyComponent greeting-message="hello" />
```

We use <span class="custom-link">[PascalCase for component tags](/components/registration/#component-name-casing)</span> when possible because it improves template readability by differentiating Vue components from native elements. However, there isn't as much practical benefit in using camelCase when passing props, so we choose to follow each language's conventions.
...
If you want to pass all the properties of an object as props, you can use <span class="custom-link">[`v-bind`](https://vuejs.org/guide/essentials/template-syntax.html#dynamically-binding-multiple-attributes)</span> <span class="custom-link">[without an argument](/essentials/template/#attribute-bindings)</span> (`v-bind` instead of `:prop-name`). For example, given a `post` object:
...
The main drawback of such mutations is that it allows the child component to affect parent state in a way that isn't obvious to the parent component, potentially making it more difficult to reason about the data flow in the future. As a best practice, you should avoid such mutations unless the parent and child are tightly coupled by design. In most cases, the child should <span class="custom-link">[emit an event](/components/events/)</span> to let the parent perform the mutation.
...
If using <span class="custom-link">[Type-based props declarations](https://vuejs.org/api/sfc-script-setup.html#type-only-props-emit-declarations)</span><sup>TS</sup>, Vue will try its best to compile the type annotations into equivalent runtime prop declarations. For example, `defineProps<{ msg: string }>` will be compiled into `{ msg: { type: String, required: true }}`.
