...
When using type-based declaration, we lose the ability to declare default values for the props. This can be resolved by using <span class="custom-link">[**`Reactive Props Destructure`**](/components/props/#reactive-props-destructure-35) <sup>3.5+</sup></span> :
...
In cases where the component referenced is a <span class="custom-link">[generic component](/typescript/overview/#generic-components)</span>, for instance `MyGenericModal`:
