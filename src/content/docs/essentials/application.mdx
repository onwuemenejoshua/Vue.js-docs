...
In-DOM templates are often used in applications that are <span class="custom-link">[using Vue without a build step](https://vuejs.org/guide/quick-start.html#using-vue-from-cdn)</span>. They can also be used in conjunction with server-side frameworks, where the root template might be generated dynamically by the server.
